...
def fasta_to_formula():
	...
	results=open('results_fasta_to_formula.txt',"a")

	#file=open(args.U).read()
	#file_splitted=file.split('>')
	#file_splitted.remove(file_splitted[0])
	...
	with open(args.U, "rU") as handle:
		for record in SeqIO.parse(handle, "fasta"):
			...
			#S=IsoSpecPy.IsoTotalProb(0.90, formula)

			#for mass, prob in S:
			#	results.write(str(mass)+", "+str(prob)+'\n')
